...
class TrShows extends _source_TrShows
{
    ...
    public function getUrl($options = null): string
    {
        ...
        return Url::to(['shows/detail', 'code' => $this->code]);
    }
     ...
     * @return string
     */
    public function getUrlTicket($params = [])
    {
    ...
    /**
     * Returns an url to tickets list for https://grandcountry.i.travel.
     *
     * @param DateTime|null $date
     *
     * @return string
     */
    public function getUrlBooking(DateTime $date = null): string
    {
        if ($date) {
            return Url::to(
                ['site/tickets', 'code' => $this->code, 'date' => $date->format('Y-m-d_H:i:s')]
            );
        }

        return Url::to(['site/tickets', 'code' => $this->code]);
    }
    ...
    {
        /*$ids = Yii::$app->params[Kiosk::KIOSK_SHOWS_IDS_PARAMS_KEY] ?? [];*/
        $cond = [self::tableName() . '.status' => self::STATUS_ACTIVE];
        /*if (!empty($ids) && Kiosk::getId() && !in_array(Kiosk::getId(), Kiosk::IDS_WITH_SHOWS, true)) {
            $cond[self::tableName() . '.id_external'] = $ids;
        }*/
        return $cond;
    }
     ...
     * @return bool
     */
    public static function fixName($ids_external, $removeText)
    {
        if (empty($removeText)) {
            return false;
        }
     ...
     * @return ActiveQuery
     */
    public static function getAllowable($code = null)
    {
    ...
    {
        return TrLocations::find()->distinct()->select(
            [TrLocations::tableName() . '.external_id', TrLocations::tableName() . '.name']
        )
            ->joinWith(
                [

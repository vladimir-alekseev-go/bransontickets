...
class Search extends Model
{
    ...
    public function getOrderBy(): array
    {
        ...
        $this->orderby = array_merge(['status' => SORT_DESC], $this->orderby);
     ...
     * @return array
     */
    public static function prepareScheduleForDay(array $scheduleByDay, DateTime $date): array
    {
        $scheduleForDay = [];
        $dateTo = clone $date;
        ...
        foreach ($scheduleByDay as $key => $it) {
            if ($it["availablePrices"]) {
                foreach ($it["availablePrices"] as $price) {
                    try {
                        $dt = new DateTime($price["start"]);
                        if ($dt >= $date && $dt < $dateTo) {
                            $scheduleForDay[!empty($price['any_time'])
                                ? 'Any Time'
                                : $dt->format("h:iA")][$it["code"]] = [
                                "start" => $price["start"],
                                "name"  => $it["name"],
                                "code"  => $it["code"],
                            ];
        ...
        uasort(
            ...
            static function ($a, $b) {
                $valA = array_values($a);
                $firstA = array_shift($valA);
                $valB = array_values($b);
                $firstB = array_shift($valB);
                if ($firstA['start'] === $firstB['start']) {
                    return 0;
                }
                return ($firstA['start'] < $firstB['start']) ? -1 : 1;
            }
        );
